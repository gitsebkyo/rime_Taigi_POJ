...
1. 基本打法
直接輸入拼音字母，隨後接著數字（1-9）來決定聲調。
範例：想打「台語」，輸入 tai5oan5。
結果：編輯區即時顯示 tâi oân，同時出現各種候選字。
...
3. 三大核心按鍵
【空格 Space】：選字推送。例如 tai5oan5 + 空格 → 出現「tâi-oân」。
【輸入 Enter】：直接輸入羅馬字。例如 tai5oan5 + Enter → 直接輸出 tâi oân（不靠詞典）。
【Shift + 數字】：強制選字。如果想選候選欄第 2 個字，按 Shift + 2（避免與聲調數字衝突）。

4. 進階技巧
翻頁：按鍵盤上的 [ 和 ]（中括號）或 Tab。
簡拼：可以只打聲母縮寫。例如輸入 t+o 也能找到「台灣」，當然打多一點比較快找到。
刪除修正：按左右方向鍵可以回頭修改特定音節的聲調，不必整串重打。
